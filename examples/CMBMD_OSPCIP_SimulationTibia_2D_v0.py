...
slurm_path = '~result/~cmbmd_ospcip_tibia'
if not os.path.exists(slurm_path): os.makedirs(slurm_path)


# FILENAMES
class pFNS:
    ProjectionID        = '~data/PROJ_Tibia_implant_60140switch_2e6gain_556pix_380x1dim.mat'  # PROJECTIONA FOR S1, S2, S3 (ASSUME #FRAME X #V X #U)
    InitialID           = '~initialization/START_CMBMD_Tibia_implant_60140switch_2e6gain_500vox_300x1x300dim.mat'  # INITIALIZATION
    ConstraintID        = '~initialization/MASK_CMBMD_Tibia_implant_60140switch_2e6gain_500vox_300x1x300dim.mat'
    GeometryID          = ''  # GEOMETRY FILE
    ReconstructionID    = slurm_path + '/CMBMD_Tibia_implant_60140switch_2e6gain_500vox_300x1x300dim.mat'  # RECONSTRUCTION (FOR FILE SAVING)
...
class pSYS:
    ...
    Atten3              = '~doc/linearatten_titanium.mat';  # LINEAR ATTENUATION OF BASIS 3
...
class pREC:
    ...
    HuberBeta3          = 1e-1;  # HUBER PENALTY STRENGTH FOR BASIS 3
...
atten3 = get_atten(pSYS.Atten3).astype(np.float32)
atten_full = np.stack((atten1[:, 1], atten2[:, 1], atten3[:, 1]), axis = -1)
...
sm.OSPCIP_make_regularization_pnorm([pREC.HuberBeta1, pREC.HuberBeta2, pREC.HuberBeta3], pnorm = 2, delta = pREC.HuberDelta)
...
constraintmasks = io.loadmat(pFNS.ConstraintID)
mask = constraintmasks['mask'].astype(np.float32)
mask.shape = sm.nVoxelZ, sm.nVoxelY, sm.nVoxelX

sm.set_y(y)
sm.OSPCIP_set_mask(mask)
...
# SET INITIAL DUAL VARIABLES, SEE S.Z. LIU'S PAPER
print('>> INITIALIZING DUAL VARIABLES...')
sm.xi = x0.copy()

tmp = np.zeros_like(sm.xi, dtype = 'float')
tmp[..., sm.mask > 0.5] = 1 / sm.xi[..., sm.mask > 0.5]
sm.Theta = tmp.copy()

sm.set_subsetslice(slice(0, sm.nView, pREC.NumSubset))
GradR, HessR = sm.Reg(sm.xi)
sm.OSPCIP_make_Grad(GradR)

tmp[..., sm.mask > 0.5] = tmp[..., sm.mask > 0.5] - sm.Grad[..., sm.mask > 0.5]
tmp = -1 * np.sum(tmp, axis = 0) / sm.nMaterial
tmp.shape = 1, sm.nVoxelZ, sm.nVoxelY, sm.nVoxelX
sm.Lamda = tmp.copy()

x0 = np.concatenate((sm.xi, sm.Lamda, sm.Theta), axis = 0)
...
solver = Nesterovify05(x0.copy(),
    generator = QuantisToolkit.optimization.optimizer_OSPCIP_generator_v0(sm,
        yeta = 0.995,
...
for i, xi in progress.iterator(enumerate(solver), num_iterations = pREC.NumIteration * pREC.NumSubset):
    if i % (20*pREC.NumSubset) == 0:
        io.savemat(pFNS.ReconstructionID[ :-4] + '_iter' + str(np.int(i/(pREC.NumSubset))) + '.mat', {'xi': xi[0 : 3, ...]})  # ONLY SAVE PRIMAL VARIABLE
    if i == pREC.NumIteration * pREC.NumSubset: break   
    if i % pREC.NumSubset: continue
    
io.savemat(pFNS.ReconstructionID, {'xi': xi[0 : 3, ...]})
print('...FINISHED!')
quit()